...
class ShapeNetPartNormal(Dataset):
    ...
    # classes = ['Sugarcane', 'Maize', 'Tomato']
    # seg_num = [2, 2, 2]
    classes = ['Sugarcane']
    seg_num = [2]
    ...
    # cls_parts = {'Sugarcane':[0,1], 'Maize':[2,3], 'Tomato': [4,5]}
    cls_parts = {'Sugarcane':[0,1]}
    cls2parts = []
    # cls2partembed = torch.zeros(16, 50)
    # cls2partembed = torch.zeros(3, 6)
    ...
    def __getitem__(self, index):
        if not self.presample:
            ...
            point_set, seg = data[:, :6], data[:, -1].astype(np.int64)
...
if __name__ == '__main__':
    train = ShapeNetPartNormal(num_points=4096, split='trainval')
    test = ShapeNetPartNormal(num_points=4096, split='test')
    for dict in train:
        for i in dict:
