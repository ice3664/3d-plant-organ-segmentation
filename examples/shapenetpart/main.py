    ...
    for idx, data in pbar:
        ...
        for shape_idx in range(batch_size):  # sample_idx
            ...
            output_preds = np.concatenate(
                (data['pos'][shape_idx].cpu().numpy(), preds[shape_idx].reshape(4096, 1).cpu().numpy()), axis=1)
            output_label = np.concatenate(
                (data['pos'][shape_idx].cpu().numpy(), data['y'][shape_idx].reshape(4096, 1).cpu().numpy()), axis=1)

            np.savetxt(os.path.join(preds_path_list, val_loader.dataset.datapath[shape_idx + 8 * idx][1].split('/')[-1]),
                       output_preds, fmt='%.5f')
            np.savetxt(os.path.join(label_path, val_loader.dataset.datapath[shape_idx + 8 * idx][1].split('/')[-1]),
                       output_label, fmt='%.5f')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, required=True, help='config file',default='')
